...
export const ContentBlock: React.FC<{ id: string, type: string, localOverrides: any }> = ({ id, type, localOverrides }) => {
    ...
    const isNavbar = type?.startsWith('B01') || type === 'Navbar';
    ...
    // Translate data for current language (skip Navbar)
    const translatedOverrides = {
        ...localOverrides,
        data: isNavbar ? localOverrides?.data : translateData(localOverrides?.data, currentLanguage)
    };
